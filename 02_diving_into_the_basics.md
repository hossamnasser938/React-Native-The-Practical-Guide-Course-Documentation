...
```
    ...
    * ` StyleSheet ` is what allows us to separate **components** *style* from the **JSX**. We use:
    ```js
    const styles = StyleSheet.create( {

    } );
    ```
    and then pass an object to ` create ` method that wraps styles of our ` components ` each as an inner object. Then we can reference those ` style ` objects in the **JSX** by ` styles.[name] `.
...
* To run our app on an android simulator:
    ...
    4. run the emulator.
    5. run your application by hitting ` react-native run-android `. Notice that you can add a script in the ` package.json ` file to specify a command you hit instead of ` react-native run-android `. Note that whatever command you choose, it should be preceded with ` npm run `. So for example if you specified your script like this: ` "android": "react-native run-android" `, you need to hit ` npm run android ` to start run your application.
...
* On running your application on Android and IOS emulators we can open the developer menu by hitting ` ctrl ` or ` cmd ` + ` m ` for **Android** or ` D `. You can use this developer menu to:
    * ` Reload `.
    * Enable ` Live Reload `.
...
* Now let's add something to our app other than this normal ` Text ` ` Component `. Let's add something to get data from user which is a ` TextInput ` ` Component ` in ` React Native `.
    ...
    ```
    Note that the value of ` style ` prop of any ` Component ` is an object which produces 2 pairs of curly braces.
        * **Hint**: If we outputted the ` state ` object the line after calling ` setstate ` method, we do not get the last value set in ` setState ` method. Instead we get the value before calling ` setState ` method which means, as I guess, ` setState ` method is **asynchronous**.  


## Styling _ Understanding the Basics
* We need to add a ` placeholder ` in the ` TextInput ` to give the user hint about what should he inputs there. Fortunately, we have a ` prop ` named ` placeholder ` that let's us specify the placeholder text. In addition to that we have a prop named ` placeholderTextColor ` that lets us specify the color of the placeholder text. So let's use these ` props `.
```js
<TextInput style={ { width:300, borderColor:"black", borderWidth:1 } }
...
* Now let's see how we position elements inside a ` Component ` or inside a wrapper ` View ` or generally in ` React Native ` apps.
    ...
    * Then we need a way to specify how the elements will be positioned in each axis.
        * ` justifyContent ` is the attribute used to specify how the elements will be positioned in the **Main axis**. ` justifyContent ` has 6  possible values:
            ...
            * ` space-evenly ` later.
        * ` alignItems ` is the attribute used to specify how the elements will be positioned in the **Cross axis**. ` alighItems ` has   possible values:
...
* Now We need to position the ` TextInput ` at the most top instead of the center. To do that only change both ` justifyContent ` to ` flex-start `. Doing this moves our ` TextInput ` to very top which leads to get it under the **status bar**. We add top ` padding ` or ` margin ` to solve this.
```js
const styles = StyleSheet.create({
  container: {
    flex: 1,
...
* Now we need to add a button next to out ` TextInput ` that lets the user adds a place.
...
* Now after we added the ` Button ` ` Add ` that is intended to let the user adds places, we hope to actually let the user does so and also display the list of places he added till now.   
    ...
    Notice that we :
        ...
        * we used ` setState ` method in a new way. We passed it not the ` newState ` but a ` function ` that accepts the ` prevState ` and returns the ` newState `. Notice also that We updated the ` prevState ` in an **immutable** way(This is important).
    ...
    ```
    4. Last in before we ` return ` in the ` render ` method, we get our ` places ` ready by wrapping each ` place ` in a ` Text ` to be easily outputted in the **JSX**. We do so using ` map ` method.
    ```js
    render() {
...
* Note that ` Text ` is not a **stylable** ` Component `. You need to wrap it in a ` View ` to apply a specific **styling**. So it will be good to define a custom ` Component ` for a list item.
    ...
    2. In ` components ` we create a file named ` ListItem.js ` to define our custom ` Component `. Since this ` Component ` will just accept a string representing a list item, no need to define it as **class-based** ` Component `. It is better to define it as **functional** ` Component `.
    ```js
    import React from 'react';
...
* Now the time for the first assignment. We see our ` App ` ` Component ` got much bigger so we need to **decompose** into multiple ` Components ` to make it more manageable.
...
* Fortunately, we have a way to make any ` Component ` **touchable** by wrapping it in a ` Component ` that a ` Touchable ` ` Component `. These ` components ` must have only one child so if you have more, wrap them in a ` View ` ` Component `. We have 4 ` Touchable ` ` components `:
...
* Now let's make our ` ListItem ` **touchable**.
    1. In ` ListItem.js ` we wrap our ` View ` in a ` Touchable ` ` Component `.
    ```js
    const ListItem = ( props ) => (
    ...
    ```
    2. In ` PlacesList.js ` we set the ` onPress ` ` prop `.
    ```js
    ...
    ```
    Here we just responded by an ` alert `. Soon we will remove that pressed item.
...
* Now instead of just ` alert ` something, we need to **remove** the place which is touched. Since places are stored in ` App ` ` Component ` ` state `, we have to define the ` function ` that removes the item touched there and then pass it to ` PlacesList ` ` Component ` to be registered with each ` ListItem ` in the list. This ` function ` will accept the **index** of the ` ListItem ` to be removed.  
    1. In ` App.js `.  
    We define the ` function `. We will use the ` filter ` method which allows us to remove the item from ` places ` array in an **immutable** way.
    ```js
...
* ` FlatList ` works in a different way than ` ScroolView `.
...
* Now let's replace our ` ScrollView ` with a ` FlatList `.
    ...
    ```
* Sometimes we have lists that are divided into **sections**. For theses lists ` SectionList ` ` Component ` is a perfect fit.
...
* Now we need to add images to our places. Let's try first to add a static image that we drag in our project to all places. Images stored locally can be ` imported ` or ` required ` and passed to ` source ` attribute of ` Image ` ` Component `.
    ...
    4. In ` ListItem.js `.
        ...
        ```
        We specified the ` width ` and `height ` because if we did not, **JS** will render the ` Image ` with its `  width ` and ` height ` which is too big. If the ` Image ` is fetched from the web instead, **JS** will hev no way to determine its ` width ` and ` height ` so we also need to specify them in that case. We also added some ` margin ` to keep the ` placeName ` away a little bit.  
        **Note** that we did not specify how the image will be resized. We actually did. ` Image ` has a ` prop ` named ` resixeMode ` and the default value of this ` prop `, which we use, is ` cover `.
        4. We also update the style of the wrapper ` View ` of the ` Image ` and the ` Text ` so that they sit bext to each other.
        ```js
        list_item: {
          width: "100%",
          flexDirection: "row",
        ...
        ```
        We do that by setting the attribute ` flexdirection ` to ` row ` instead of the default ` column ` and align ` Text ` center horizontally by setting the ` alighItems ` attribute to ` center ` which is specific to the **Cross axis** which is the horizontal axis here.


## Using Network Image
* Now we we need to **fetch** our ` Image ` from the web instead of dragging it in our project. All we need to change is the object given to ` source ` attribute. Now no need to ` import ` anything, we just assign this ` source ` attribute an object with one attribute named ` uri ` andset it to the **URL** of he ` Image `. In ` App.js ` remove the ` import ` of the local image and assign this atribute:
```js
addPlaceHandler = () => {
  if( this.state.placeName.trim() !== "" ) {
    this.setState( prevState => {
...
## Adding a Modal
* A ` Modal ` is a ` Component ` that can be viewed on top of other ` component `. A typical use case for a ` Modal ` is when you want the user on tapping on an ` Image `, it appears in a the whole screen.
